...
export const SELECTION_SYNC_LOCK_MS = 200;
...
export const createSelectionSlice: SelectionSliceCreator = (set, get) => ({
  ...
  syncSelectionFromReactFlow: (nodeIds: string[], edgeIds: string[]) => {
    ...
    const isSameSingleEdgeSelection =
      nodeIds.length === 0 &&
      edgeIds.length === 1 &&
      state.selectedEdgeIds.length === 1 &&
      state.selectedEdgeId === edgeIds[0];
    const preserveMemberSelection =
      isSameSingleEdgeSelection &&
      (state.selectedMemberLinkIndices.length > 0 || state.selectedLagId !== null);
    ...
    set({
      ...
      selectedMemberLinkIndices: preserveMemberSelection ? state.selectedMemberLinkIndices : [],
      selectedLagId: preserveMemberSelection ? state.selectedLagId : null,
    });
  },
