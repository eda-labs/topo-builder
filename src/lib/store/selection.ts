...
export const createSelectionSlice: SelectionSliceCreator = (set, get) => {
  const selectEdgeAndClearNodeSelection = (edgeId: string, extra: {
  ...
  return {
    ...
    selectMemberLink: (edgeId: string, index: number | null, addToSelection?: boolean) => {
      ...
      selectEdgeAndClearNodeSelection(edgeId, { selectedMemberLinkIndices: newIndices, selectedLagId: null });
    },

    selectLag: (edgeId: string, lagId: string | null) => {
      selectEdgeAndClearNodeSelection(edgeId, { selectedMemberLinkIndices: [], selectedLagId: lagId });
    },
